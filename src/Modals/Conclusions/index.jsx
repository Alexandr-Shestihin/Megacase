...
const Conclusions = (props) => {
   ...
   const getData = useCallback((value, count = 10, page = 1) => {
      ...
      if (typeof value === 'string' && maxPages >= page) { // Использован оператор >
         ...
         API.getHistorySkins(page, count, value)
            .then(response => {
               const newItems = response.data; // Получаем новые элементы
               if (newItems.length > 0) { // Проверяем, есть ли новые элементы
                  ...
                  setData((prev) => {
                     const newData = [...prev, ...newItems]; // Добавляем только новые элементы
                  ...
                  // Если новых элементов нет, можно установить maxPages в текущую страницу,
                  // чтобы остановить useIntersectionObserver
                  setMaxPages(page);
               }
   ...
   }, [maxPages, setMaxPages]); // setLoadStatus убран, так как он не влияет на логику останова запросов
   ...
   return (
      <div className={s.container}>
         <div className="pageTitle">{t("total.historyConclusions")}
            <span className="pageSubtitle">{t("total.skins")}</span>
         </div>
         <div className="row mt16" onClick={handler}>
            <Button id={'all'} active={activeID === 'all' && true} className={`${s.btn} btn-text`} title='Trade URL' >{t("total.all")}</Button>
            <Button id={'received'} active={activeID === 'received' && true} className={`${s.btn} btn-text`} title='Accepted' >{t("total.received")}</Button>
            <Button id={'sent'} active={activeID === 'sent' && true} className={`${s.btn} btn-text`} title='Sented' >{t("total.sent")}</Button>
            <Button id={'error'} active={activeID === 'error' && true} className={`${s.btn} btn-text`} title='Error' >{t("total.error")}</Button>
         </div>
         <div className={`mt12 ${s.itemContainer}`}>
            {data?.length > 0 ? (
               data.map(el => (
                  <ConclusionsItem key={el?.id} name={el?.itemName} price={el?.price} updatedAt={el?.updatedAt} status={el?.displayStatus} image={el?.image} />
               ))
            ) : (
               loadStatus?.message
            )}
